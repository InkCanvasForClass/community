...
namespace Ink_Canvas.Ink.WetInk
{
    ...
    /// </summary>
    internal static class WetInkTailPredictor
    {
        ...
        /// 实时书写路径应改用 <see cref="WetInkPredictionSmoother"/>。
        /// </summary>
        public static IReadOnlyList<WetInkPredictedPoint> Build(IReadOnlyList<WetInkRealPoint> realPoints)
        {
            if (!TryEstimateMotion(realPoints, out var motion))
                return Array.Empty<WetInkPredictedPoint>();
        ...
        /// </summary>
        internal static IReadOnlyList<WetInkPredictedPoint> Build(
            IReadOnlyList<WetInkRealPoint> realPoints,
            WetInkPredictionSmoother smoother)
        {
            if (smoother == null)
        ...
        /// </summary>
        private static WetInkMotion RescaleToSpeed(WetInkMotion motion, double targetSpeed)
        {
            if (motion.Speed <= 0.0001 || targetSpeed <= 0)
                return motion;

            var scale = targetSpeed / motion.Speed;
            return new WetInkMotion(
        ...
        private static IReadOnlyList<WetInkPredictedPoint> Extrapolate(
            IReadOnlyList<WetInkRealPoint> realPoints,
            WetInkMotion motion,
            int pointCount,
            ...
            double curvature)
        {
            var result = new List<WetInkPredictedPoint>(pointCount);
            ...
            for (var i = 0; i < pointCount; i++)
            {
                ...
                if (traveled + stepDistance >= distanceCap)
                {
                    if (stepDistance > 0.0001)
                    {
                        ...
                        if (IsFinite(endX) && IsFinite(endY) && t > 0.001)
                        {
                            ...
                            result.Add(new WetInkPredictedPoint(
                                endX,
                                endY,
                ...
                result.Add(new WetInkPredictedPoint(currX, currY, predictedPressure, stamp));
            }
        ...
        /// </summary>
        private static double ComputeSpeedBaselineMilliseconds(WetInkMotion motion)
        {
        ...
        /// </summary>
        private static double ComputeStaleScale(IReadOnlyList<WetInkRealPoint> realPoints)
        {
            var last = realPoints[realPoints.Count - 1];
            var secondLast = realPoints[realPoints.Count - 2];
        ...
        /// </summary>
        private static double ComputeTurnScale(IReadOnlyList<WetInkRealPoint> realPoints)
        {
        ...
        /// 三点近似共线或退化时返回 0。
        /// </summary>
        private static double EstimateSignedCurvature(IReadOnlyList<WetInkRealPoint> realPoints)
        {
        ...
        private readonly struct WetInkMotion
        {
            public WetInkMotion(
                double velocityX,
                double velocityY,
    ...
    /// </summary>
    internal sealed class WetInkPredictionSmoother
    {
            ...
            IReadOnlyList<WetInkRealPoint> realPoints)
        {
            ...
            var alpha = 1.0 - Math.Exp(
                -deltaMilliseconds / WetInkTailPredictor.CurvatureTauMilliseconds);
            ...
            IReadOnlyList<WetInkRealPoint> realPoints)
        {
            ...
            var tau = targetHorizonMicroseconds < _previousHorizonMicroseconds
                ? WetInkTailPredictor.SlowdownTauMilliseconds
                : WetInkTailPredictor.AccelerationTauMilliseconds;
